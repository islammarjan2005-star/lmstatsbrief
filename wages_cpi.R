...
find_latest_cpi_period <- function(pg_data) {
  cpi_data <- pg_data %>%
    filter(
      earnings_type == CPI_CURRENT$TOTAL_EARNINGS_TYPE,
      earnings_metric == CPI_CURRENT$EARNINGS_METRIC,
      time_basis == CPI_CURRENT$TIME_BASIS
    ) %>%
  ...
  if (nrow(cpi_data) == 0) return(NULL)

  cpi_data$parsed_date[1]
}
...
compute_wages_cpi <- function(pg_data) {
  # Find latest period dynamically
  anchor_m <- find_latest_cpi_period(pg_data)

  if (is.null(anchor_m)) {
    empty <- list(cur = NA_real_, dq = NA_real_, dy = NA_real_,
                  dc = NA_real_, de = NA_real_,
                  pct_vs_dec2007 = NA_real_, pct_vs_pandemic = NA_real_)
...
calculate_wages_cpi <- function() {
  pg_data <- fetch_wages_cpi()
  compute_wages_cpi(pg_data)
}
