...
find_latest_sector_period <- function(pg_data, sic_section) {
  sector_data <- pg_data %>%
    filter(sic_section == !!sic_section) %>%
...
compute_sector <- function(pg_data, anchor, sic_section) {
  if (is.null(anchor) || is.na(anchor)) {
...
calculate_sector_payroll <- function() {
  pg_data <- fetch_sector_payroll()

  # Find latest period dynamically (use hospitality as reference)
  anchor <- find_latest_sector_period(pg_data, SECTOR_CODES$HOSPITALITY)

  list(
    hospitality = compute_sector(pg_data, anchor, SECTOR_CODES$HOSPITALITY),
    retail = compute_sector(pg_data, anchor, SECTOR_CODES$RETAIL),
    health = compute_sector(pg_data, anchor, SECTOR_CODES$HEALTH)
  )
}
